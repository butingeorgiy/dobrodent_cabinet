    ...
    render(results, observeLinkListener, mode) {
        ...
        if (patients.length > 0) {
            ...
            patients.forEach(patient => {
                this.container.innerHTML += `
                    <a href="/${mode}/patients/${patient.id}" class="global-search-item observer-link flex px-4 py-2 hover:bg-gray-50 cursor-pointer">
        ...
        if (doctors.length > 0) {
            ...
            doctors.forEach(doctor => {
                this.container.innerHTML += `
                    <a href="/${mode}/doctors/${doctor.id}" class="global-search-item observer-link flex px-4 py-2 hover:bg-gray-50 cursor-pointer">
                        <div class="min-w-10 min-h-10 w-10 h-10 mr-2 sm:mr-4 bg-center bg-no-repeat bg-contain rounded-full border border-gray-300"
        ...
        if (visits.length > 0) {
            ...
            visits.forEach(visit => {
                this.container.innerHTML += `
                    <a href="/${mode}/visits/${visit.id}" class="global-search-item observer-link flex px-4 py-2 hover:bg-gray-50 cursor-pointer">
