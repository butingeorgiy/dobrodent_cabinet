...
@section('content')
                    ...
                    <div class="flex lg:flex-col items-center w-full">
                        <div class="flex mr-auto">
                            ...
                            <div class="flex flex-col">
                                <a href="{{ route('administrator-doctor', ['id' => $doctor['id']]) }}" class="mb-1 text-green-light sm:text-xl font-medium leading-5 hover:underline">{{ $doctor['full_name'] }}</a>
                                <i class="mb-4 text-gray-400 text-sm sm:text-base">{{ $doctor['occupation'] }}</i>
                            </div>
                        </div>
                        <div class="flex items-center self-start ml-3.5 text-gray-400">
    ...
    <div class="container mx-auto flex justify-center mt-8">
        <button class="show-more-doctors-btn transition duration-300 text-gray-500 border border-gray-500 rounded-md text-sm px-3 py-2 sm:px-5 hover:text-white hover:bg-gray-500 focus:outline-none">Показать больше</button>
    </div>
@endsection
