...
@section('content')
    <div class="w-full mb-4 bg-white shadow-md p-4 select-none">
        <div class="flex">
            ...
            <a href="{{ route('doctor-visits-create') }}" class="flex items-center ml-auto px-3 py-2 sm:px-5 border border-indigo-100 bg-indigo-100 text-sm text-white rounded-md transition duration-300 ease select-none hover:bg-indigo-200 hover:border-indigo-200 focus:outline-none">
                <svg class="w-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round"
                          stroke-linejoin="round"
                          stroke-width="2"
                    ...
                    <div class="py-4 text-sm sm:text-base">
                        ...
                        @if($visit->patient)
                            <div class="flex mb-1">
                                ...
                                <a href="{{ route('doctor-patient', ['id' => $visit->patient->id]) }}" class="text-gray-500 hover:underline">{{ $visit->patient->last_name . ' ' . mb_substr($visit->patient->first_name, 0, 1) . '.' . mb_substr($visit->patient->middle_name, 0, 1) . '.' }}</a>
                            </div>
                        @endif
                        @if($visit->cause)
                            <div class="flex">
