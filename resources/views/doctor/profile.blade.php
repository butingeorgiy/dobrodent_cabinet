...
@section('content')
    <div class="mx-auto sm:max-w-xl bg-white shadow py-5">
        <p class="px-4 text-xl sm:text-2xl text-gray-400 select-none">Личные данные</p>
        <hr class="my-4" />
        <form action="{{ route('doctor-profile-update') }}" method="POST" enctype="multipart/form-data" id="doctorProfileForm" class="flex flex-col px-4 pt-4">
            <label class="mx-auto mb-2 rounded-full border-4 border-gray-300 w-32 h-32 sm:w-40 sm:h-40 bg-no-repeat bg-center bg-cover cursor-pointer"
            ...
            <i class="text-xs mx-auto mb-6 text-gray-400 select-none">Ред. фотографию</i>
            <label class="flex flex-col mb-4">
                <span class="mb-1 font-medium text-gray-700 select-none">Имя</span>
                <input class="border border-gray-300 rounded-md px-3 py-2 outline-none text-gray-500" type="text" value="{{ $doctor->first_name }}" name="first_name" placeholder="Введите ваше имя" required>
            </label>
            <label class="flex flex-col mb-4">
                <span class="mb-1 font-medium text-gray-700 select-none">Фамилия</span>
                <input class="border border-gray-300 rounded-md px-3 py-2 outline-none text-gray-500" type="text" value="{{ $doctor->last_name }}" name="last_name" placeholder="Введите вашу фамилию" required>
            </label>
            <label class="flex flex-col mb-4">
                <span class="mb-1 font-medium text-gray-700 select-none">Отчество</span>
                <input class="border border-gray-300 rounded-md px-3 py-2 outline-none text-gray-500" type="text" value="{{ $doctor->middle_name }}" name="middle_name" placeholder="Введите ваше отчество" required>
            </label>
            <label class="flex flex-col mb-4">
                <span class="mb-1 font-medium text-gray-700 select-none">E-mail</span>
                <input class="border border-gray-300 rounded-md px-3 py-2 outline-none text-gray-500" type="text" name="email" value="{{ $doctor->email }}" placeholder="Введите ваш e-mail" required>
            </label>
            <label class="flex flex-col mb-4">
            ...
            <label class="hidden flex flex-col px-4 py-3 bg-green-light bg-opacity-30">
                ...
                <span class="flex">
                    <button id="confirmDoctorProfileBtn" class="mr-2 px-4 py-2 border border-green-light bg-green-light text-sm sm:text-base text-white rounded-md transition duration-300 ease select-none hover:bg-green-light hover:border-green-light focus:outline-none focus:ring-4 focus:ring-green-light focus:ring-opacity-50">
                        Сохранить
                    </button>
            ...
            @csrf
            <button id="saveDoctorProfileBtn" type="button" class="mt-4 px-4 py-2 border border-indigo-100 bg-indigo-100 text-sm sm:text-base text-white rounded-md transition duration-300 ease select-none hover:bg-indigo-200 hover:border-indigo-200 focus:outline-none focus:ring-4 focus:ring-indigo-100 focus:ring-opacity-50">
                Сохранить изменения
            </button>
        </form>
    </div>
