...
@section('content')
    ...
    <div class="patients-search-container flex flex-col shadow-md w-full select-none">
        @foreach($patients as $patient)
            <div class="flex patient-item flex-col md:flex-row {{ $patient['was_visit'] ? 'bg-yellow-50' : 'bg-white' }} p-4 border-b border-gray-200 last:border-0">
                <div class="flex items-center mr-auto">
                    ...
                    <div class="flex flex-col">
                        <a href="{{ route('doctor-patient', ['id' => $patient['id']]) }}" class="text-green-light sm:text-xl font-medium sm:mb-1 hover:underline">{{ $patient['full_name'] }}</a>
                        <i class="text-gray-400">{{ $patient['phone'] }}</i>
                    </div>
                </div>
                <div class="flex items-center mt-4 sm:mt-6 md:mt-0 w-full md:w-auto">
                    <a href="{{ route('doctor-visits-create', ['patient' => $patient['id']]) }}"
