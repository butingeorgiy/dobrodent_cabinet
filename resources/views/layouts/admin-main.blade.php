...
<div class="sticky top-0 z-40">
    <header class="bg-white">
        <div class="container flex items-center mx-auto px-4 sm:px-8">
            <a class="mr-auto py-4" href="{{ route('administrator-index') }}">
            ...
            <div class="flex items-center py-4 relative">
                <div class="hidden sm:flex flex-col text-right mr-5 select-none">
                    <p class="font-bold text-gray-500">
                        @php
                            $admin = App\Facades\Authorization::user();

                            echo $admin->name;
                        @endphp
                    </p>
                    <span
                        class="open-extra-dropdown-menu-btn flex items-center justify-end text-sm font-medium text-gray-400 cursor-pointer hover:opacity-70">
                        <svg class="mr-1 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                             stroke="currentColor">
                ...
                <div
                    ...
                    <a class="flex px-4 py-2 text-red-500 hover:bg-gray-100" href="{{ route('administrator-logout') }}">
                        ...
                        <p class="whitespace-nowrap">Выйти из системы</p>
                    </a>
                </div>
            </div>
        </div>
    ...
    <nav class="hidden xl:block bg-green-light xl:pt-4 xl:pb-4 transition-max-h duration-500 ease-linear max-h-0 xl:max-h-full xl:h-auto overflow-hidden">
        <div class="container flex flex-col xl:flex-row xl:justify-between mx-auto px-4 sm:px-8 xl:px-40 select-none">
            <a class="flex mb-2 xl:mb-0 items-center text-white {{ !request()->is('administrator') ? 'opacity-60' : '' }} hover:opacity-100"
               href="{{ route('administrator-index') }}">
                <svg class="mr-1 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                     stroke="currentColor">
                    <path stroke-linecap="round"
                          stroke-linejoin="round"
                          stroke-width="2"
            ...
            <a class="flex mb-2 xl:mb-0 items-center text-white {{ !request()->is(['administrator/doctors', 'administrator/doctors/*']) ? 'opacity-60' : '' }} hover:opacity-100"
               href="{{ route('administrator-doctors') }}">
                <svg class="mr-1 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                     stroke="currentColor">
                    <path stroke-linecap="round"
                          stroke-linejoin="round"
            ...
            <a class="flex mb-2 xl:mb-0 items-center text-white {{ !request()->is('administrator/visits', 'administrator/visits/*') ? 'opacity-60' : ''  }} hover:opacity-100"
               href="{{ route('administrator-visits') }}">
                <svg class="mr-1 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                     stroke="currentColor">
                    <path stroke-linecap="round"
                          stroke-linejoin="round"
            ...
            <a class="flex mb-2 xl:mb-0 items-center text-white {{ !request()->is('administrator/patients', 'administrator/patients/*') ? 'opacity-60' : '' }} hover:opacity-100"
               href="{{ route('administrator-patients') }}">
                <svg class="mr-1 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                     stroke="currentColor">
                    <path stroke-linecap="round"
                          stroke-linejoin="round"
